...
{{-- Edit Cv Form --}}
<form class="form-horizontal" method="post" action="@if (isset($cv)){{ URL::to('user/cvs/' . $cv->id . '/edit') }}@endif" autocomplete="off">
    <!-- CSRF Token -->
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane" id="tab-user-info">
            ...
            <div class="col-xs-4 {{{ $errors->has('cv.owner.gender') ? 'error' : '' }}}">

                <label class="control-label" for="meta-title">Gender</label>
<!--                <input class="form-control" type="text" name="cv.owner.gender" id="cv.owner.gender" value="{{{ Input::old('cv.owner.gender', isset($cv->owner->gender) ? $cv->owner->gender : null) }}}" />
                {{{ $errors->first('cv.owner.gender', '<span class="help-block">:message</span>') }}}-->
{{ Form::text('cv.owner.gender') }}
            </div>
